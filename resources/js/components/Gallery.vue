<template>
    <div class="gallery" :class="{editable}">
        <cropper v-if="field.type === 'media' && editable" :image="cropImage" @close="cropImage = null"
                 @crop-completed="onCroppedImage" :configs="field.croppingConfigs"/>

        <component :is="draggable ? 'draggable' : 'div'" v-if="images.length > 0" v-model="images"
                   class="gallery-list clearfix">

            <component :is="singleComponent" v-for="(image, index) in images" class="mb-3 p-2 mr-3"
                       :key="index" :image="image" :field="field" :editable="editable" :dowloadable="downloadable"
                       :removable="editable" @remove="remove(index)"
